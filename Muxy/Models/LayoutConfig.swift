...
struct LayoutConfig: Equatable {
    ...
    static func directory(forProjectPath projectPath: String) -> URL {
        primaryDirectory(forProjectPath: projectPath)
    }

    static func primaryDirectory(forProjectPath projectPath: String) -> URL {
        URL(fileURLWithPath: projectPath)
            .appendingPathComponent(".roost")
            .appendingPathComponent("layouts")
    }

    static func legacyDirectory(forProjectPath projectPath: String) -> URL {
        URL(fileURLWithPath: projectPath)
    ...
    static func directories(forProjectPath projectPath: String) -> [URL] {
        [
            primaryDirectory(forProjectPath: projectPath),
            legacyDirectory(forProjectPath: projectPath),
        ]
    }
    ...
    static func discover(projectPath: String) -> [LayoutDescriptor] {
        ...
        for directory in directories(forProjectPath: projectPath) {
            ...
            for entry in entries where allowed.contains(entry.pathExtension.lowercased()) {
                ...
                if descriptorsByName[name] == nil {
                    descriptorsByName[name] = LayoutDescriptor(name: name, url: entry)
                }
            }
        }
