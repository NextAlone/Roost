...
struct LayoutConfigParsingTests {
    ...
    @Test("discovers roost layouts before legacy muxy layouts")
    func discoversRoostLayoutsFirst() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("roost-layouts-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let roostLayouts = root
            .appendingPathComponent(".roost", isDirectory: true)
            .appendingPathComponent("layouts", isDirectory: true)
        let muxyLayouts = root
            .appendingPathComponent(".muxy", isDirectory: true)
            .appendingPathComponent("layouts", isDirectory: true)
        try FileManager.default.createDirectory(at: roostLayouts, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: muxyLayouts, withIntermediateDirectories: true)
        try "tabs:\n  - roost\n".write(to: roostLayouts.appendingPathComponent("default.yaml"), atomically: true, encoding: .utf8)
        try "tabs:\n  - legacy\n".write(to: muxyLayouts.appendingPathComponent("default.yaml"), atomically: true, encoding: .utf8)
        try "tabs:\n  - old\n".write(to: muxyLayouts.appendingPathComponent("legacy.yaml"), atomically: true, encoding: .utf8)

        let layouts = LayoutConfig.discover(projectPath: root.path)

        #expect(layouts.map(\.name) == ["default", "legacy"])
        #expect(layouts.first(where: { $0.name == "default" })?.url.path.contains(".roost/layouts/default.yaml") == true)
    }
}
