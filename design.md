...
- **License 决策**：先以 **Apache-2.0** 作为 default 目标（允许商用、专利条款明确、兼容大多数依赖）。仅当引入 GPL-only 依赖时（当前 cmux 参考的是图片 asset，不算代码）再评估切换 GPL-3.0。发布第一版前锁 LICENSE 文件 + 每个源文件头加 SPDX 标识。
...
- **跨平台**：仅 macOS。Windows/Linux **永不**在本项目。
- **VCS**：MVP 只支持 jj。git worktree 见 M12。
- **内置浏览器 / SSH / 远程执行**：后续（M13 仅 hostd 远端，非浏览器集成）。
- **diff viewer / 深度 IDE 集成**：后续（M11），先靠 `open -a` 握手。
- **多人协作 / 云同步**：不在路线内。
- **Accessibility 全面适配**（VoiceOver / Dynamic Type）：v1.0 前仅 "能用"，不做增强。
- **i18n**：硬编码中文 + 必要处英文。v1.0 不拆 strings 文件。
- **App Sandbox**：不开（见 §11 entitlements）。MAS 发行视为非目标。
...
| **M17** | 键绑定自定义 | M6 |
| **M18** | 三层 Sidebar（Project → Workspace → Session）+ workspace 新建 UI | M6 + M2 |

M6-M8 是 daemon 迁移的**三步串行**，不要并行；M5 / M10 / M11 / M12 与它们正交，可穿插。M14-M17 都独立可做，但**依赖 M6 hostd**——配置真相源必须落 hostd，否则 app 和 CLI 看到不同的值。M18 同样等 M6，因为 workspace 元数据需要由 hostd 统一 poll/缓存，app 和 CLI 才能共享一致视图。

### M18：三层 Sidebar（待 M6 合入后开始）

**动机**: 当前 Sidebar 只分 Project → Session 两层，workspace 信息仅在 session label 后缀（`agent@workspace`）。设计文档 §3 的 `AgentSession.workspace_id` 已假定 workspace 是一等实体；UI 需要同步升级。
...
│   ▾ @ui-b        ⚠ divergent        │
│       ✨ claude                     │
│   + New workspace…                  │  ← 弹 LauncherSheet 预填 project
│ ▾ 📁 Quark                          │
│   ▾ @default                        │
│       🖥 shell                     │
└────────────────────────────────────┘
...
**数据层改动** (hostd 侧，M6 已有基础):
- 新 RPC `list_workspaces(project_id) -> Vec<WorkspaceDto>`。
- hostd 周期性（`jj workspace list` 每 N 秒 / 每次 session create 后触发）刷新每个 project 的 workspace 列表，缓存到内存 + SQLite。
- `WorkspaceDto` 字段：`id / name / path / bookmark / is_current / current_revision (short+desc) / divergent_flag / last_sync_at`。
- `session.workspace_id` 关联到 workspace row（M6 之前 session 只记 `workingDirectory`，迁移脚本按路径回填）。

**Sidebar 重构**:
- 数据源从 `(Project, [Session])` 改为 `(Project, [(Workspace, [Session])])`。
- `SidebarBucket` 拆成 `ProjectBucket` / `WorkspaceBucket` / `SessionRow`，三级 collapse 状态独立持久化。
- workspace 行点击 = 切 current bucket 到该 workspace（session filter 收紧到 workspace 内）。
- TabBar 同步改：过滤 sessions by `selectedWorkspaceID` 而不是 projectID。
- 顶层 "Scratch" 保留（无 workspace 概念）。

**新建 workspace UI**:
- Project 折叠区最后一项 "+ New workspace" 按钮（以及 project row 右键 "New jj workspace…"）。
- 弹现有 `LauncherSheet`，预填 `form.target = project.id`、`form.useJjWorkspace = true`、焦点在 workspace name 字段。
- Launch 走现有 `addWorkspace` + setup hooks + session spawn 流程。

**Divergent 检测**（design.md §5 已预留）:
- 后台 sync 任务对比 `workspace.bookmark` 的 head 和 `@` 所在 change。
- 不一致 → flag `divergent = true`，sidebar 显示 ⚠ + tooltip 说明。
- "Reconcile" 右键菜单项：`jj bookmark set <ws>-head -r @` 并 refresh。

**与 M11 diff viewer / M12 git worktree 的配合**:
- M11: workspace 行右键 "Show diff" → 基于该 workspace 的 `jj diff`，不受当前 session 影响。
- M12: `trait Vcs` 抽象 `list_workspaces` / `create_workspace` / `delete_workspace`；git worktree 实现映射到 `git worktree list / add / remove`。

**出站标准**:
- 打开 app 能看到每个 project 下所有 jj workspace，即使没有 session 挂载。
- 新建 workspace 从 Sidebar "+ New workspace" 一步完成，且 sidebar 立即显示新 workspace 行（hostd 推 `workspace_created` 事件）。
- divergent workspace 可视化 + 一键修复。
...
| Gatekeeper 拒启动（未 notarize 或签名被撤） | 高 | §12 CI 走 notarize；撤销应急靠备用 cert；用户侧 `xattr -d com.apple.quarantine` 临时绕（仅开发构建提示） |
| Sparkle 自动更新被劫持 | 高 | EdDSA 签名（不用 DSA）；appcast 走 HTTPS；public key embed 到 app，换 key 要 major bump |
| 依赖 jj / agent CLI 未装 | 中 | §12.5 first-run 检测；运行时路径解析失败给明确错误而非静默 |
| Xcode / macOS SDK 漂移 | 中 | CI pin macos-14；project.yml + XcodeGen 锁；deploymentTarget 不跟最新 SDK 走 |
| Crash 报告泄露敏感路径 | 中 | §11.3 redact 规则：`$HOME` → `~`、env var 白名单、panic message 截首 256 字节 |
...
| 层 | 范围 | 工具 | gate |
|---|---|---|---|
| **Rust unit** | `hooks.rs` / `jj.rs` / 未来 `rpc`, `session`, `pty` | `cargo test` | CI 必 pass；每次 jj 调用路径扩展时加 case |
| **Rust integration** | hostd 启动 / socket RPC round-trip / adopt / quit | `tests/` 目录 + `tokio::test` | M6 起可用；一个 fake jj binary 替身（env var 切换） |
| **Swift unit** | `HookStepResult.parseAll` / `RoostBridge` 解析 / `IDEOpener.installed()` stub | XCTest | M6 后加；先 IME state machine / LauncherForm |
| **Manual smoke** | IME 切换 / 多 project / workspace CRUD / agent crash → UI 反馈 | 跑前写成 check list | 每个 milestone 收尾时跑一次 |

硬规：**M6 开工前** `tests/` 骨架要搭好；`jj.rs` 269 行当前零覆盖，daemon 拆分前至少盖 `list_workspaces` + `add_workspace` 的 parse path（预防 jj 输出格式变更带连锁 regression）。

### 11.2 CI/CD

GitHub Actions workflow 分三档：

- **`pr.yml`** (每 PR)：`cargo fmt --check`、`cargo clippy -D warnings`、`cargo test`、`xcodebuild -project apps/Roost -scheme Roost build`，macOS 14 runner。
- **`main.yml`** (push to main)：以上 + 产 unsigned .app artifact，7 天保留。
- **`release.yml`** (tag `v*`)：codesign + notarize + 打 dmg，上传到 GitHub Releases；见 §12。

`cargo sqlx prepare` 一旦 M6 起用 sqlx 就进 pre-commit + CI gate（metadata 必须 check in）。`xcodegen` 不在 CI 跑，project.yml 改动必须本地重新 generate 后一起 commit（避免 CI 跑出跟作者机器不同的 pbxproj）。

### 11.3 Crash 上报

- **Rust (hostd)**：启用 `[profile.release] debug=1` 保符号；`#[panic_handler]` 写 minidump 到 `~/Library/Application Support/roost/crashes/`；Sentry 或自托管 Sentry-compatible（待定）。panic 捕获位置：RPC handler、PTY read task、tokio runtime root。
- **Swift (app)**：`NSSetUncaughtExceptionHandler` + Signal handler (SIGSEGV/SIGBUS/SIGABRT) 写同一目录。发送前弹窗"发送崩溃报告？" 用户 opt-in。
- 上报流量：**仅在明确 opt-in 后发送**，默认 off。上报前文本审查（redact path / env var）。v1.0 前可以先纯本地 dump，不带网络。

### 11.4 代码风格

- **Rust**: `cargo fmt` (default config) + `cargo clippy -D warnings`。
- **Swift**: `swift-format` 或 `SwiftLint` 之一（待定），default rules + 覆盖 3 条：120 列、trailing_comma 允许、monospaced-font 识别。
- **commit message**: `<type>(<scope>): <subject>`，type ∈ `feat / fix / docs / refactor / perf / test / chore`；scope 小写模块名（`sidebar` / `term` / `m5` / ...）。

### 11.5 macOS entitlements

**不** 开 App Sandbox。原因：
- spawn `jj` / agent CLI / shell 要任意 PATH + 任意 cwd，sandbox `com.apple.security.temporary-exception.files.home-relative-path.read-write` 维护成本高。
- hostd 需要绑 UDS 文件到 `~/Library/Application Support/roost/hostd.sock`，sandbox container 下的 socket 路径对 CLI / 远程连接不透明。
- 目标分发渠道是**直接下载 / Homebrew cask**，不走 Mac App Store。

但 **Hardened Runtime + Notarization 必开**（macOS Gatekeeper 要求）：
- 签名 identity：Developer ID Application。
- 关键 entitlement：
  - `com.apple.security.cs.allow-jit`（ghostty Metal renderer 需要）
  - `com.apple.security.cs.allow-unsigned-executable-memory`（Rust static lib 与 ghostty 的内存模型）
  - `com.apple.security.automation.apple-events`（`open -a` 跳第三方 IDE / Terminal）
- 不需要：camera / microphone / location / network client-server（hostd UDS 是本地，不受 sandbox 网络限制）。

hostd 作为**独立二进制**嵌入 app bundle 的 `Contents/MacOS/roost-hostd`；签名沿用 app identity，不需要独立 provisioning。

### 11.6 `jj` 硬约束（与 repo 策略一致）

- push 前签名预检：任一 change `UNSIGNED` 即阻塞，补签修复。
- 禁 `jj rebase -r <rev>`（re-parents descendants），统一 `jj rebase -b @ -d <target>` 或 `jj retrunk`。
- `jj squash` 必须带 `-m`（否则进交互式编辑器挂 UI）。
...
| 渠道 | 触发 | 产物 |
|---|---|---|
| **GitHub Releases** | tag `v*` | 带签名的 `.dmg`，附 `Roost-<version>-arm64.dmg` 和 `Roost-<version>-universal.dmg` |
| **Homebrew Cask** | 每次 release 后手工提 cask formula PR | `homebrew-roost` tap 或上游 `homebrew-cask` |
| **Mac App Store** | 非目标 | — |
| **TestFlight** | 非目标 | — |

### 12.2 Signing / Notarization 流程

```
1. xcodebuild archive -configuration Release ...
2. xcodebuild -exportArchive ... -exportOptionsPlist ExportOptions.plist
   → 产 .app，含 Developer ID 签名
3. codesign --deep --force --sign "Developer ID Application: <team>" --options runtime \
            --entitlements Roost.entitlements Roost.app
4. notarytool submit Roost-<v>.zip --apple-id ... --password ... --team-id ... --wait
5. stapler staple Roost.app
6. create-dmg ... → Roost-<v>-arm64.dmg
7. codesign dmg + notarytool submit dmg + stapler staple
```

CI secrets：`APPLE_ID` / `APPLE_ID_PASSWORD` (app-specific) / `TEAM_ID` / `CERT_P12_BASE64` / `CERT_P12_PASSWORD`，存 GitHub Actions encrypted secrets。

### 12.3 自动更新

采用 [Sparkle 2](https://sparkle-project.org/)，EdDSA 签名（不用旧 DSA）：

- 构建时 embed `SUPublicEDKey`（Sparkle 要求）。
- Release workflow 生成 `appcast.xml` 发到 `https://roost.sh/appcast.xml`（或 GitHub Pages）。
- app 默认每 24h 检查；用户可在 Settings 关闭。
- 首次发布前决定：更新频率、channel（stable / beta）、rollback 策略。v1.0 只给 stable 单 channel。

### 12.4 版本号

Semver：`0.y.z` pre-1.0 期间 y 每 milestone bump，z 补丁；1.0 起严格 semver。
- hostd / app / CLI 三个产物共享同一 `CARGO_PKG_VERSION`（`roost-core` 作为 source of truth）。
- major 不匹配 hostd ↔ app 拒 adopt（§5a）。

### 12.5 First-run UX

首次启动 flow（M6 前靠现状，M6 后加 onboarding）：
1. 检测 `jj` 是否在 PATH；若否弹一次性提示 "please install jj (brew install jj)"，不阻塞。
2. 检测 `~/Library/Application Support/roost/` 是否存在；不存在则创建 + 默认 config.json。
3. project 列表为空时 Sidebar 显示"Add your first project"引导（现已 partial：`No projects yet` 文案）。
4. 第一个 project add 后弹引导 tooltip 介绍 ⌘T / ⌃1 / ⌃2 三键（出现一次后不再显示，记 `config.ui.onboarded = true`）。
