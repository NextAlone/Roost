...
### M-1：libghostty 可行性 POC（最高优先级）

**做什么**：一个最小 SwiftUI app，嵌入 libghostty，打开一个 pane 跑 `bash`，键盘输入回显、ANSI 颜色显示正常。**零 Rust 参与**。

**为什么 M-1**：libghostty 的 C API 上游明确标注 unstable，cmux 靠 track main 分支吃住了这条路。如果这步跑不通（头文件缺、链接失败、渲染异常），整个架构都要改（换 SwiftTerm 或 alacritty_terminal + Metal）。

**交付**：
- 仓库下 `pocs/libghostty-hello/` 子目录。
- 跑起来能见黑底白字的 shell。
- README 记录：libghostty 从哪来（Homebrew / 自行 build）、头文件路径、链接步骤。
...
- **M0.0**：M-1 之上增加 PTY 管理：Swift `posix_spawn` 一个子进程，stdin/stdout/stderr 走 PTY，libghostty 渲染。验证 PTY 路径、信号处理、exit 回收。
- **M0.1**：建 Rust core + swift-bridge 的 hello world。`greet(name) -> String` 在 Swift 按钮里调通。与 libghostty 无关。
- **M0.2**：接合。Swift 调 Rust `prepare_session("claude")`，拿到 argv + env，按 M0.0 的路径 spawn 起来。SQLite 只存 sessions 一张表。
