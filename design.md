...
- **M0.0**：借 M-1 的 libghostty 路径，改 `surface_config.command` 跑一个具体 agent（`claude` / `codex`）而不是登录 shell，验证任意 CLI agent 都能在 ghostty surface 里跑。
- **M0.1**：建 Rust core + swift-bridge 的 hello world。`greet(name) -> String` 在 Swift 按钮里调通。与 libghostty 无关。
- **M0.2**：接合。Swift 调 Rust `prepare_session("claude")`，拿到 argv + env，按 M0.0 的路径启动到 ghostty surface。SQLite 只存 sessions 一张表。
