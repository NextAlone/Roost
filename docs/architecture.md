...
- **Hostd Live Attach**: In hostd-owned runtime mode, Roost starts a standalone `roost-hostd-daemon` helper and talks to it over the private Unix socket `/tmp/roost-hostd-daemon-$uid.sock`. The daemon, not the app-bundled XPC service, owns agent PTYs so sessions can survive the Roost UI process exiting. The launcher accepts an existing daemon only when its `HostdDaemonRuntimeIdentity` protocol version matches the app; older daemons on the same socket are treated as unavailable so the current bundled helper can replace the socket. `RoostHostdCore.HostdProcessRegistry` opens the macOS PTY, spawns the command via `posix_spawn` as a new session with the PTY slave as its controlling terminal, drains output into a bounded byte ring buffer, and asynchronously feeds those bytes into a SwiftTerm-backed headless terminal snapshot cache. Stream reads can request raw byte limits for diagnostic consumers and carry an end-of-stream marker after the session process exits. Reattached helpers first request the latest completed terminal snapshot frame, which resets transient terminal modes including synchronized output, serializes the visible screen, alternate-screen state, cursor position, and cell attributes into bounded VT output at the raw sequence covered by that cached frame; subsequent reads resume raw live bytes after that sequence, so snapshot parsing lag cannot block PTY draining or attach streaming. Exited sessions remain readable long enough for helpers to drain final output and exit normally. Roost bootstraps a daemon-backed `RoostHostdClient` with a bounded runtime check, publishes the client before replaying restored panes, and replays any hostd-owned panes created before client readiness. Roost keeps hostd-owned panes in a preparing state only while session creation is in flight, moves them to failed when daemon calls time out or error, and renders ready panes through normal `GhosttyTerminalNSView` surfaces by launching `roost-hostd-attach --session <id>` inside the pane. The helper bridges local stdin/stdout directly to the daemon socket while the daemon remains the PTY owner. Roost does not maintain a separate production VT renderer for hostd-owned panes; `RoostHostdXPCService` remains available for metadata-only compatibility and development fallback.
