...
Muxy/
  ...
  Services/
    ...
    Config/
      ...
      RoostAppConfigStore.swift     Reads/writes compatibility app config under Application Support
      AgentPresetResolver.swift     Resolves app-wide agent presets with project env
  ...
  Views/
    ...
    Settings/
      ...
      RoostConfigSettingsView.swift Project .roost/config.json notification and permission UI
```
...
- **Persistence**: App state uses `MuxyFileStorage` under `~/Library/Application Support/Roost/` for projects, workspace slots, shortcuts, custom command shortcuts, Ghostty config, notifications, and the notification socket. General preferences follow the upstream pattern and live in `UserDefaults`, including the default parent folder for new workspaces and the app-wide hostd runtime mode. Project config lives in-repo at `{Project.path}/.roost/config.json`; legacy setup commands still fall back to `{Project.path}/.muxy/worktree.json`. Named pane/tab layouts live at `{Project.path}/.roost/layouts/<name>.{yaml,yml,json}` with legacy discovery fallback to `{Project.path}/.muxy/layouts/`. Roost writes project config through `RoostConfigStore`, creating `.roost/` as `0700` and the config file as `0600`. New managed workspaces use `GeneralSettingsKeys.defaultWorktreeParentPath` when present, grouped by sanitized project name, and project-specific workspace locations still win. Roost env values may reference macOS Keychain items by service and optional account; setup, teardown, and agent tab launches resolve those references at runtime and do not write secret values back to config. Teardown commands run before managed workspace removal and do not block cleanup when a command fails. Host/session history lives under `~/Library/Application Support/Roost/hostd/`. `RoostHostdXPCService` defaults to `.appOwnedMetadataOnly`; `ROOST_HOSTD_RUNTIME` still overrides startup for development. `RoostAppConfigStore` remains a compatibility/advanced source for shared agent env and app-wide agent presets, but it no longer controls default workspace placement or hostd runtime selection.
- **Hostd Live Attach**: In hostd-owned runtime mode, Roost starts a standalone `roost-hostd-daemon` helper and talks to it over the private Unix socket `/tmp/roost-hostd-daemon-$uid.sock`. The launcher accepts an existing daemon only when its `HostdDaemonRuntimeIdentity` protocol version matches the app; older daemons on the same socket are treated as unavailable so the current bundled helper can replace the socket. For agent panes (`agentKind != .terminal`), `RoostHostdCore.HostdProcessRegistry` records the session metadata but delegates terminal ownership to `tmux`: it launches `tmux new-session -d -s roost-<session-id> -c <workspace> -- <agent command>` with the resolved Roost environment, lets tmux own pane `TERM`, sets fixed RGB terminal features for Ghostty / xterm clients, and sets `mouse on`, `status off`, and `prefix` / `prefix2` to `None` on the Roost-owned session. Restored sessions are checked through `tmux has-session`, terminated through `tmux kill-session`, and rendered through normal `GhosttyTerminalNSView` surfaces by configuring the same options before `tmux attach-session -t roost-<session-id>`. This makes detach, reattach, scrollback, alternate screen, terminal mode state, and output backpressure the responsibility of the mature multiplexer instead of a Roost ANSI replay bridge, while avoiding tmux's status bar and hidden prefix key capture inside Roost panes. If `tmux` is missing or returns an error during creation, the pane moves to failed with the tmux error instead of staying in preparing. Ordinary terminal panes still use Roost's existing PTY implementation inside `HostdProcessRegistry`: macOS PTY creation, `posix_spawn`, output ring buffering, SwiftTerm-backed snapshots, stream reads, resize, input writes, and signal forwarding remain available for tests, diagnostics, and non-agent hostd APIs. The old app-local attach bridge is no longer built or launched. `RoostHostdXPCService` remains available for metadata-only compatibility and development fallback.
- **Agent Reload**: For tmux-backed agent panes, hostd sets `remain-on-exit on` on the Roost-owned session and watches `pane_dead` after the agent exits. When the agent dies, hostd captures the last 200 lines of the pane via `tmux capture-pane -p -S -200 -N -J`, persists the captured tail to `SessionRecord.lastTail` (SQLite schema v2), and surfaces the captured string back to the app. The app extracts a resume command per-`AgentKind` using `AgentKind.defaultResumeRegex` (overridable per preset via `RoostConfigAgentPreset.resumeCommandRegex`). Resume strategy varies: Claude Code uses `.appendArgs` (preset's default command + `--resume <id>`); Codex uses `.replaceWithCaptured` (the captured `codex resume <id>` verbatim, dropping preset top-level flags); Gemini and OpenCode are `.notSupported` and fall back to fresh restart. `AppState.reloadAgent` orchestrates the full flow: optional SIGINT escalation via `AgentReloadCoordinator` for running agents, terminate old hostd session, remove the ghostty NSView from the registry, replace `pane.sessionID` with a fresh UUID. SwiftUI's `.id(pane.sessionID)` on the ghostty representable then drives a clean recreation of the NSView. UI surfaces are an exit banner, a binary-update banner triggered on pane focus by mtime comparison, and pane context-menu items "Reload Agent (Resume)" / "Reload Agent (Fresh)".
...
`JjPanelView` renders Files, Changes, Bookmarks, Operation Log, and Conflicts sections. The Changes section keeps the two-line row style: description on the first line, then change id, commit id, author, relative time, and bookmark labels on the second line. The left graph column uses each `jj log` change row's raw graph prefix plus its attached continuation lines inside the same row block, and intentionally does not insert standalone graph-only continuation rows. `JjGraphGlyphLayout` groups the raw curved graph text into jj's two-character glyph cells, and `JjGraphGlyphBlock` maps those cells to rounded vector paths so branch connectors stay visually continuous while preserving the exact `jj log` topology. Its width ignores trailing graph padding so branch connectors stay close to the change metadata. Its header can apply a free-form revset to reload that graph. Rows use commit ids internally for SwiftUI identity and action targets so divergent change ids remain visible without ambiguous mutations. Mutating actions serialize through `JjProcessQueue.shared` via `JjMutationService` and refresh the panel after success. Current actions include describe, new, commit, squash, abandon, duplicate, backout, edit, rebase working copy, bookmark create, bookmark move, bookmark rename, bookmark delete, fetch tracked bookmarks, push a selected bookmark, conflict content viewing, conflict open-in-editor handoff, conflict resolution through `jj resolve --tool :ours/:theirs`, and restore repository state to a selected operation via `jj op restore --what repo`.

The jj panel intentionally does not expose every legacy Git operation. Remaining future work includes an optional embedded three-way conflict editor and richer DAG navigation beyond the current raw log rendering.
