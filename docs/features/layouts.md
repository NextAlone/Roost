...
Roost can apply named pane/tab layouts to a workspace on demand. Layouts live in-repo under `{Project.path}/.roost/layouts/` so they can be checked in alongside the project. Legacy `{Project.path}/.muxy/layouts/` files are still discovered as a fallback.

## Behavior

- Each file in `.roost/layouts/` defines one named layout. The file name without extension is the layout's name.
...
A Roost workspace is a tree of panes inside a single window. Each leaf pane is a stack of tabs (one tab visible at a time). Panes can be nested with horizontal or vertical splits.
...
The examples below can be saved under `.roost/layouts/` and double as a reference for the schema. Each diagram shows the resulting window with panes drawn as boxes; tabs are listed at the top of their pane.
...
The same schema works as JSON at `.roost/layouts/<name>.json`:
