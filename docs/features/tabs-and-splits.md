...
The tab and split tree per workspace is in-memory only. To recreate a layout, use [Layouts](layouts.md) — a declarative `.roost/layouts/<name>.yaml` file you can apply on demand. Legacy `.muxy/layouts` files still work as a fallback.
