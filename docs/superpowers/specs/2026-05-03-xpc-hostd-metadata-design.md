...
These methods are present in the shared protocol and client surface, but `.appOwnedMetadataOnly` rejects them explicitly. Later hostd-owned slices implemented PTY ownership, output streaming, input forwarding, resize, signal delivery, and keepalive behavior behind the same boundary. Visible live attach is now covered by `2026-05-03-hostd-live-attach-design.md`: Roost mounts normal `GhosttyTerminalNSView` surfaces and launches `roost-hostd-attach --session <id>` as the pane command instead of keeping a separate production VT renderer in the app.
