...
These methods are present in the shared protocol and client surface, but `.appOwnedMetadataOnly` rejects them explicitly. `HostdProcessRegistry` now proves the hostd-owned PTY core path in isolation: it opens the PTY, launches the command, owns the master fd, returns attach metadata with `attachedClientCount`, reads bounded output, writes stdin, resizes the PTY, sends interrupt signals, validates release calls against active attaches, retains a keepalive while each PTY session is running, releases that keepalive on explicit teardown or natural process exit, and marks the stored history record exited. `RoostHostdXPCService` can exercise that path when `ROOST_HOSTD_RUNTIME=hostd-owned-process` is set or when app-wide `hostdRuntime` in `~/Library/Application Support/Roost/config.json` is `hostdOwnedProcess`; default service startup remains metadata-only. In hostd-owned mode the service backs the registry keepalive with an XPC transaction so the service is not idle-terminated while owned PTY sessions are alive. The app now preserves the runtime ownership hint and renders hostd-owned agent panes with attach/release lifecycle calls, a bounded UTF-8 output view, ANSI SGR color/intensity rendering, raw PTY input bridge, geometry-driven PTY resize, and Ctrl-C interrupt forwarding instead of mounting `TerminalBridge`, which prevents duplicate local Ghostty process launch. Live Ghostty attach remains future UI work because the bundled GhosttyKit surface API currently accepts command/cwd/env inputs but does not expose a public existing PTY fd adoption entry point.
